      ...
      <div class="product-msg">
        <ul>
          ...
          <li v-for="item in goodsList" :key="item.id">
            ...
            <p>${{item.salePrice}}</p>
            <div class="add-mg">
            <a href="javascript:" @click="addCart(item.productId)"><img src="./../assets/cart1.svg" alt="" style="margin-left:40px;width:25px;height:25px;margin-left:40px;"></a>
            </div>
          </li>
        </ul>
        <!-- 无限滚动 -->
...
export default {
  ...
  methods: {
    ...
    getGoodList(flag) {
      ...
      axios.get("/goods/clothes", { params: param }).then(response => {
    ...
    detail(productId){
     ...
     priceselect(startPrice,endPrice){
      ...
      console.log(endPrice);
       axios.get("/goods/clothesselect", { params: param }).then(response => {
        let res = response.data;
        this.goodsList=res.result.list;
         if(this.goodsList==''){
