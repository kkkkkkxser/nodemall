<template>
    ...
    <div class="detail">
        ...
        <div class="addr-detail">
            <ul>
                <li v-for="(item,index) in addressListFilter" :key="item.id" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index">
                    <dl>
                        <dt>{{item.userName}}</dt>
                ...
                <li><a href="javascript:avoid(0)" @click="addA"><img src="./../assets/add.svg" class="add-p"/></a>
                <p>添加新地址</p>
                </li>
            </ul>
        ...
        <!-- 商品信息 -->
        <div class="product">
            <div class="product-text"><a>商品</a></div>
            <div class="product-msg">
                <ul>
                    <li v-for="(item,index) in payList" :key="item">
                        <div class="pro-img">
                            <img v-lazy="require(`./../assets/${item.productType}/${item.productImage}`)" alt="">
                            <div class="pro-img-tx">
                            <a>全球限量顶级{{item.productName}}</a>
                            </div>
                        </div>
        ...
        <!-- 配送方式 -->
        <div class="send-way">
            <a class="big">配送方式</a><a class="small">包邮</a><br>
            <a class="big">发票</a><a class="small-1">电子发票 个人</a>
        </div>
        <!-- 总价 -->
        <div class="total">
            <div class="total-text">
            <a class="total-left">商品件数:</a><a class="total-right">{{totalNum}}</a><br>
            <a class="total-left">商品总价:</a><a class="total-right">{{total}}</a><br>
            <a class="total-left">优惠活动:</a><a class="total-right">0</a><br>
            <a class="total-left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;运费:</a><a class="total-right">0</a><br>
            <a class="total-left">应付总额:</a><a class="total-right">{{total}}</a><br>
            </div>
        </div>
        <!-- 结算 -->
        <div class="buttons">
    ...
    <el-dialog title="添加地址" :visible.sync="dialogFormVisible">
      <el-form :model="addressA">
       ...
       <el-form-item label="地址" :label-width="formLabelWidth" class="regist-form">
          <el-input v-model="addressA.address" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="手机号" :label-width="formLabelWidth" class="regist-form">
          <el-input v-model="addressA.phone" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
